...
$reservation_staff_memo = egb('reservation_staff_memo');

if (!isset($reservation_staff_memo)) {
    $reservation_staff_memo = '';
}

// 예약 확정 처리를 위한 업데이트 쿼리 - 그룹 내 모든 예약 데이터 업데이트
$update_query = "
    UPDATE egb_reservation 
    SET reservation_status = 2,
        reservation_confirmed_at = NOW(),
        reservation_confirmed_by = 'system',
...
$update_params = [
    ...
    ':reservation_manager_note' => $reservation_staff_memo
];
...
$query = "
    SELECT r.*, 
           ...
           MAX(r.reservation_confirmed_at) as reservation_confirmed_at,
...
$result = [
    ...
    'manager_note' => $reservation_staff_memo,
...
// 사용자에게 예약 확정 알림 전송
$message = "{$store_name}의 {$formatted_date} {$formatted_time} 예약이 확정되었습니다.";
$view_path = "";
