...
$query = "INSERT INTO egb_board_mentoring_release (
    uniq_id,
    board_user_uniq_id,
    board_title,
    board_contents,
...
if ($result) {
    // 레코드 카운트 증가
    increase_record_total_count('egb_board_mentoring_release');
    increase_record_active_count('egb_board_mentoring_release');

    // 게시판 로그 추가
    egb_board_log('egb_board_mentoring_release', $uniq_id, $user_uniq_id);

    // 리워드 지급
    egb_reward_dispatch($user_uniq_id, 'reward_mentoring_release_upload', 'point', 0, 'auto_grade_check');

    // sitemap 추가
    egb_sitemap_insert('mentoring_release', DOMAIN . '/page/mentoring_release_view/?uniq_id=' . $uniq_id);

    echo json_encode(['success' => true, 'successCode' => 16, 'url' => DOMAIN . '/page/mentoring_release_view/?uniq_id=' . $uniq_id]);
} else {
    // 게시글 삽입 실패 시 처리
