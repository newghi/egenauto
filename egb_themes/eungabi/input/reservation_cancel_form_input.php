...
$reservation_cancel_memo = egb('reservation_cancel_memo', 159);

// 예약 취소 처리를 위한 업데이트 쿼리 - 그룹 내 모든 예약 데이터 업데이트
$update_query = "
    UPDATE egb_reservation 
    SET reservation_status = 0,
        reservation_canceled_at = NOW(),
        reservation_canceled_by = 'system',
        reservation_canceled_note = :reservation_canceled_note,
        updated_at = NOW(),
        updated_by = 'system'
...
$update_params = [
    ...
    ':reservation_canceled_note' => $reservation_cancel_memo
];
...
$query = "
    SELECT r.*, 
           ...
           MAX(r.reservation_canceled_at) as reservation_canceled_at
...
// 사용자에게 예약 취소 알림 전송
$message = "{$store_name}의 {$formatted_date} {$formatted_time} 예약이 취소되었습니다.";
$view_path = "";
...
egb_send_push_event(
    ...
    [
        'uniq_id' => $reservation_group_uniq_id,
