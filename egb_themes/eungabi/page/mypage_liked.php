...
// 내가 좋아요한 게시글 동적 출력 (여러 게시판, 최신순)
$my_liked_posts = [];
if ($user_uniq_id) {
    // 1. 내가 좋아요한 게시글의 게시판명, uniq_id 목록 조회
    $query = "SELECT like_target_table, like_target_uniq_id FROM egb_like_log WHERE like_user_uniq_id = :user_id AND like_type = 1 AND is_status = 1 ORDER BY created_at DESC LIMIT 10";
    $binding = binding_sql(0, $query, [':user_id' => $user_uniq_id]);
    $like_list = egb_sql($binding);
    ...
    $actual_like_data = [];
    if (isset($like_list[0]) && is_array($like_list[0])) {
        foreach ($like_list[0] as $item) {
            if (is_array($item) && isset($item['like_target_table']) && isset($item['like_target_uniq_id'])) {
                $actual_like_data[] = $item;
            }
        }
    }
    
    // 게시판별로 그룹핑
    foreach ($actual_like_data as $row) {
        $table = $row['like_target_table'];
        $uniq_id = $row['like_target_uniq_id'];
        if (!isset($board_like_map[$table])) $board_like_map[$table] = [];
        $board_like_map[$table][] = $uniq_id;
    }

    // 3. 각 게시판별로 게시글 정보 조회
    foreach ($board_like_map as $board_table => $uniq_ids) {
    ...
    usort($my_liked_posts, function($a, $b) {
