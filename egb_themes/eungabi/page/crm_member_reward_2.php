...
SELECT u.*, p.* 
FROM egb_user u
LEFT JOIN egb_point p ON u.uniq_id = p.point_target_uniq_id AND p.is_status = 1
WHERE 1=1
";
...
if ($crm_member_reward_viewing_period == 'today') {
    ...
    $query .= " AND DATE(p.created_at) BETWEEN :created_at_start AND :created_at_end";
    ...
    $params[':created_at_start'] = $created_at_start;
    $params[':created_at_end'] = $created_at_end;
} else if ($crm_member_reward_viewing_period == 'period_search') {
...
// 정렬 설정 - no 컬럼 기준으로 변경
$query .= " AND p.point_target_uniq_id IS NOT NULL ORDER BY p.no DESC";
...
foreach($results[0] as $row) {
    if($row['point_type'] == '1') {
        $positive_points += (int)$row['point_amount'];
    } else if($row['point_type'] == '0') {
        $negative_points += (int)$row['point_amount']; 
    }
}
...
<section class="position1 width_box height_box">
    <div class="flex_fl width_box height_box padding_px-l_200" data-xy="1-1200: flex_ft padding_px-l_000">
        ...
        <div class="width_box height_box padding_px-a_020" data-bg-color="#E6E6E5">
            <div class="flex_xs1_yc padding_px-u_020"
                ...
                <div class="font_px_020 flv6">적립금조회</div>
                <div class="flex_xc" data-xy="1-800: flex_xr, 801-1200: flex_xc">
                    <div class="flex_xs1_yc width_px_300 font_px_016 padding_px-u_000" data-color="#888888"
                        ...
                        <div>리워드</div>
                        <div>></div>
                        <div class="flv6" data-color="#000000">적립금관리</div>
                    </div>
                </div>
            </div>
            ...
            document.addEventListener('DOMContentLoaded', function() {
                // 적립금 조회기간 라디오 버튼 이벤트 리스너 
                const rewardPeriodRadios = document.querySelectorAll('input[name="crm_member_reward_viewing_period"]');
                ...
                rewardPeriodRadios.forEach(radio => {
                ...
                function handleDateSelection(value) {
                    const dateFields = {
                        year1: document.getElementById('crm_member_reward_year_1'),
                        month1: document.getElementById('crm_member_reward_month_1'), 
                        day1: document.getElementById('crm_member_reward_day_1'),
                        year2: document.getElementById('crm_member_reward_year_2'),
                        month2: document.getElementById('crm_member_reward_month_2'),
                        day2: document.getElementById('crm_member_reward_day_2')
                    };

                    // 모든 날짜 필드 초기화 및 비활성화 설정
                    Object.values(dateFields).forEach(el => {
                        el.value = '';
                ...
                function setDates(startDate, endDate) {
                    ...
                    // 시작 날짜 설정
                    dateFields.year1.value = startDate.getFullYear();
                    dateFields.month1.value = String(startDate.getMonth() + 1).padStart(2, '0');
                    dateFields.day1.value = String(startDate.getDate()).padStart(2, '0');

                    // 종료 날짜 설정
                    dateFields.year2.value = endDate.getFullYear();
                    dateFields.month2.value = String(endDate.getMonth() + 1).padStart(2, '0');
                    dateFields.day2.value = String(endDate.getDate()).padStart(2, '0');
            ...
            <div class="font_px_020 flv6 padding_px-y_020">조회기간 내 적립금 내역 통계</div>
    ...
    <div class="width_box padding_px-y_015 flex_xc font_px_016 flv6 border_px-u_001" data-bg-color="#efefef"
        data-bd-a-color="#d9d9d9">가용적립금</div>
    <div class="flex_fl width_box height_box" data-xy="1-800: flex_ft" data-bg-color="#ffffff">
        <div class="flex_fl_yc width_box font_style_center" data-xy="1-800: flex_ft">
            <div class="width_box flex_ft border_px-r_001 border_px-u_000" data-bd-a-color="#d9d9d9"
                ...
                <div id="pay1" class="padding_px-y_015"><?php echo number_format($positive_points); ?></div>
            </div>
            <div class="width_box flex_ft border_px-r_001 border_px-u_000" data-bd-a-color="#d9d9d9"
                data-xy="1-800: border_px-r_000 border_px-u_001">
                <div class="padding_px-y_015 border_px-u_001 flv6" data-bg-color="#efefef"
                    data-bd-a-color="#d9d9d9">차감</div>
                <div id="pay2" class="padding_px-y_015"><?php echo number_format($negative_points); ?></div>
            </div>
            <div class="width_box flex_ft" data-bd-a-color="#d9d9d9">
                <div class="padding_px-y_015 border_px-u_001 flv6" data-bg-color="#efefef"
                    data-bd-a-color="#d9d9d9">합계</div>
                <div id="pay_sum" class="padding_px-y_015"><?php echo number_format($total_points); ?></div>
            </div>
        </div>
    </div>
</div>
            <div class="font_px_020 flv6 padding_px-y_020">적립금 조회결과</div>
                ...
                <div class="flex_xs1_yc" data-xy="1-800: flex_fu">
                    <div class="flex_fl_yc padding_px-y_010 padding_px-x_015 font_px_014"
                        ...
                        <div class="" data-color="#888888">총&nbsp;<span class="flv6"
                                data-color="#15376b"><?php echo $total_count; ?></span>&nbsp;건&nbsp;중&nbsp;검색&nbsp;결과&nbsp;<span
                                class="flv6" data-color="#15376b"><?php echo $search_count; ?></span>&nbsp;건
                        </div>
...
if ($results && isset($results[0])) {
    foreach ($results[0] as $row) {
        // 포인트 잔액 계산
        $point_before = $row['point_before_balance'] ?? 0;
        $point_amount = $row['point_amount'] ?? 0; 
        $point_type = $row['point_type'] ?? '0';
        $point_after = $row['point_after_balance'] ?? 0;

        // 지급/차감 금액 표시
        $reward = $point_type == '1' ? number_format($point_amount) : '-';
        $deduction = $point_type == '0' ? number_format($point_amount) : '-';
        ...
        <div class="grid_xx border_px-u_001" data-xx="5% 5% 9% 9% 10% 10% 10% 10% 8% 19% 5%" data-bd-a-color="#d9d9d9" data-bg-color="#ffffff">
            ...
            <div class="flex_xc border_px-r_001 padding_px-y_015" data-bd-a-color="#d9d9d9">' . number_format($point_after) . '</div>
            <div class="flex_xc border_px-r_001 padding_px-y_015" data-bd-a-color="#d9d9d9">' . ($row['created_by'] ?? '') . '</div>
            <div class="flex_xc border_px-r_001 padding_px-y_015" data-bd-a-color="#d9d9d9">' . ($row['point_reason'] ?? '') . '</div>
