...
// 마이페이지 주인 정보 (예시: 본인 프로필)
// 실제로는 해당 페이지의 프로필 대상 uniq_id를 받아야 함
$profile_uniq_id = $user_uniq_id; // 본인 프로필 기준
...
if ($user_uniq_id) {
    ...
    $binding = binding_sql(1, $query, [':user_id' => $user_uniq_id]);
    $result = egb_sql($binding);
    $scrap_count = isset($result[0]['cnt']) ? $result[0]['cnt'] : 0;

    // 내 좋아요 카운트
    $query = "SELECT COUNT(*) as cnt FROM egb_like_log WHERE like_user_uniq_id = :user_id AND like_type = 1 AND is_status = 1";
    $binding = binding_sql(1, $query, [':user_id' => $user_uniq_id]);
    $result = egb_sql($binding);
    $like_count = isset($result[0]['cnt']) ? $result[0]['cnt'] : 0;
}

// 내가 작성한 게시글 동적 출력 (여러 게시판, 최신순)
$my_posts = [];
if ($user_uniq_id) {
    // 1. 내가 작성한 게시글의 게시판명, uniq_id 목록 조회 (egb_board_log 테이블 사용)
    $query = "SELECT log_target_table, log_board_uniq_id FROM egb_board_log WHERE log_user_uniq_id = :user_id AND is_status = 1 ORDER BY created_at DESC LIMIT 4";
    $binding = binding_sql(0, $query, [':user_id' => $user_uniq_id]);
    $board_log_list = egb_sql($binding);
    
    // 2. 게시판별로 uniq_id 그룹핑
    $board_posts_map = [];
    
    // 3중 배열 구조에서 실제 데이터 추출
    $actual_board_log_data = [];
    if (isset($board_log_list[0]) && is_array($board_log_list[0])) {
        foreach ($board_log_list[0] as $item) {
            if (is_array($item) && isset($item['log_target_table']) && isset($item['log_board_uniq_id'])) {
                $actual_board_log_data[] = $item;
            }
        }
    }
    
    // 게시판별로 그룹핑
    foreach ($actual_board_log_data as $row) {
        $table = $row['log_target_table'];
        $uniq_id = $row['log_board_uniq_id'];
        if (!isset($board_posts_map[$table])) $board_posts_map[$table] = [];
        $board_posts_map[$table][] = $uniq_id;
    }

    // 3. 각 게시판별로 게시글 정보 조회
    foreach ($board_posts_map as $board_table => $uniq_ids) {
...
if ($user_uniq_id) {
    ...
    $query = "SELECT scrap_target_table, scrap_target_uniq_id FROM egb_scrap_log WHERE scrap_user_uniq_id = :user_id AND scrap_type = 1 AND is_status = 1 ORDER BY created_at DESC LIMIT 4";
    $binding = binding_sql(0, $query, [':user_id' => $user_uniq_id]);
...
<div class="width_box">
    <div class="width_px_1220 margin_x_auto" data-xy="0-1220: width_box">
        <div class="flex_xs1_yt padding_px-y_030" data-xy="0-1240: flex_ft_yc padding_px-x_040">
            <div class="flex_ft_yc width_px_270 height_px_430 border_px-a_001" data-bd-a-color="#dadce0" data-xy="0-1240: width_box height_px_300">
                ...
                <div class="width_box" data-xy="0-1240: flex_xs2_yc padding_px-x_020">
                    <div class="" data-xy="0-1240: flex_fl_yc">
                        ...
                        <div class="flex_xc_yc width_box padding_px-t_030 padding_px-u_015 padding_px-x_040" data-xy="0-1240: padding_px-a_040">
                ...
                <div class="flex_xs3_yc width_box height_box padding_px-x_020 padding_px-y_020">
                    ...
                    <div class="flex_ft_xc_yc width_box">
                        ...
                        <div class="flex_xc_yc width_box padding_px-t_005 font_px_014">
                            스크랩 <?php echo isset($scrap_count) ? $scrap_count : 0; ?>
                            <?php if (isset($is_scrapped) && $is_scrapped): ?> <span class="font_px_012" data-color="#4d3e7a">(내가 스크랩함)</span><?php endif; ?>
                        </div>
                    </div>
                    <div class="flex_ft_xc_yc width_box">
                        <div class="flex_xc_yc width_box">
                        ...
                        <div class="flex_xc_yc width_box padding_px-t_005 font_px_014">
                            좋아요 <?php echo isset($like_count) ? $like_count : 0; ?>
                            <?php if (isset($is_liked) && $is_liked): ?> <span class="font_px_012" data-color="#4d3e7a">(내가 좋아요함)</span><?php endif; ?>
                        </div>
                    </div>
                </div>
            </div>
            <div class="flex_ft_yc">
                <div class="width_px_820 margin_px-u_080" data-xy="0-820: width_box">
                    <div class="flex_xs1_yc width_box padding_px-t_020 margin_px-a_010">
                        <div class="font_px_018 flv6">나의 게시글</div>
                        <div class="flex_xr_yc padding_px-x_020">
                            <div class="font_px_014 flv6 pointer" data-color="#15376b">전체보기</div>
                        </div>
                    </div>
                    <div class="flex_xs3_yc width_box">
                        <?php if (!empty($flat_my_posts)): ?>
                            <?php foreach ($flat_my_posts as $post): ?>
                            ...
                            <?php endforeach; ?>
                            <?php
                            $remaining = 4 - count($flat_my_posts);
                    ...
                    <div class="flex_xc_yc width_box padding_px-t_020">
                        <a href="/page/board_write" class="width_box margin_px-x_010">
                            <div class="flex_xc_yc width_box padding_px-x_020 padding_px-y_010 border_bre-a_005 border_px-a_001 pointer" data-bd-a-color="#dadce0">
                                ...
                                <span class="padding_px-x_010 padding_px-y_010 font_px_016 flv6" data-color="#292D32">글쓰기</span>
                            </div>
                        </a>
                    </div>
                </div>
                <div class="width_px_820 margin_px-u_080">
                    ...
                    <div class="flex_xs1_yc width_box padding_px-t_020 margin_px-a_010">
                        <div class="font_px_018 flv6">내가 저장한 게시글</div>
                        <div class="flex_xr_yc padding_px-x_020">
                            <div class="font_px_014 flv6 pointer" data-color="#15376b">전체보기</div>
                        </div>
                    </div>
                    <div class="flex_xs3_yc width_box">
                        <?php if (!empty($flat_posts)): ?>
                            <?php foreach ($flat_posts as $post): ?>
                                <div class="flex_ft_xc_yc width_box margin_px-a_010">
                                    <div class="width_box height_px_185 border_bre-a_010" data-bg-color="#f5f5f5">
                                        <?php if (!empty($post['board_thumbnail_url'])): ?>
                                            <a href="/page/board_view?table=<?php echo urlencode($post['board_table']); ?>&id=<?php echo urlencode($post['uniq_id']); ?>" class="width_box height_box display_block pointer" style="text-decoration:none;">
