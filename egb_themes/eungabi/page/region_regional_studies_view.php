...
if (!$login_user_uniq_id) {
    echo "<script nonce=" . NONCE . " type='text/javascript'>window.location.href='" . DOMAIN . '/page/region_regional_studies_preview/?uniq_id=' . $uniq_id . "'; </script>";
    exit;
}

// 게시물 데이터를 조회하는 쿼리
$query_post = "SELECT p.*, u.user_mentor_id 
               FROM egb_board_region_regional_studies p
...
if (!isset($sql_post[0]['uniq_id'])) {
    echo "<script nonce=" . NONCE . " type='text/javascript'>window.location.href='" . DOMAIN . '/page/region_regional_studies_list' . "'; </script>";
    exit;
}
...
$query = "SELECT COUNT(*) as like_count 
         FROM egb_like_log 
         WHERE like_target_table = 'egb_board_region_regional_studies'
...
$query = "SELECT COUNT(*) as scrap_count 
         FROM egb_scrap_log
         WHERE scrap_target_table = 'egb_board_region_regional_studies'
    ...
    <div class="padding_px-x_010">
        <div class="width_px_720 margin_x_auto padding_px-y_050" data-xy="1-800: width_box">
            ...
            <div class="grid_xx padding_px-t_030 padding_px-u_060 display_off" data-xx="25% 25% 25% 25%"
                ...
                // 최근 5개의 게시물을 가져오는 쿼리
                $query_post = "
    SELECT * FROM egb_board_region_regional_studies 
                ...
                if (isset($sql_post[0]) && !empty($sql_post[0])) {
                    ...
                    for ($i = 0; $i < min(3, $post_count); $i++) {
                        ...
                        <a class="padding_px-a_010"
                            href="<?php echo DOMAIN . '/page/region_regional_studies_view/?uniq_id=' . $post['uniq_id']; ?>">
                    ...
                    if ($post_count > 3) {
                        ?>
                        <a class="padding_px-a_010" href="<?php echo DOMAIN . '/page/region_regional_studies_plus_view'; ?>">
                            <div class="position1 height_px_180 width_box border_bre-a_005 overflow_hidden pointer a"
                                data-xy="1-500: height_px_150, 501-800: height_px_200">
...
//프로팅 출력
    ...
    auto_comment('egb_comment_region_regional_studies', $uniq_id, $user_uniq_id); 
    auto_floating('egb_board_region_regional_studies', $uniq_id, 'region_regional_studies');

 //조회수 1증가
 $query = "UPDATE egb_board_region_regional_studies SET board_view = board_view + 1 WHERE uniq_id = :uniq_id";
