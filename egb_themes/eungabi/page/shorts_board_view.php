...
if (isset($user_login)) {
    ...
    echo "<script nonce=" . NONCE . " type='text/javascript'>window.location.href='" . DOMAIN . '/page/shorts_board_preview/?uniq_id=' . $uniq_id . "'; </script>";
    exit;
}
...
if (!isset($sql_post[0]['uniq_id'])) {
    echo "<script nonce=" . NONCE . " type='text/javascript'>window.location.href='" . DOMAIN . '/page/shorts_board_list' . "'; </script>";
    exit;
}
...
<section class="position1 width_box height_box">
    <div class="padding_px-x_010">
        <div class="width_px_720 margin_x_auto padding_px-y_050" data-xy="1-800: width_box">
            <div class="video-container position-relative width_box" style="position: relative; padding-bottom: 56.25%;">
                <!-- 비디오가 로드되기 전에는 이미지가 표시됩니다 -->
                <img id="shorts_main_img" src="<?php echo DOMAIN . $post_data['thumbnail']; ?>"
                    class="shorts_main_img width_box height_box" style="object-fit: cover; position: absolute; top: 0; left: 0;">
                <!-- 비디오가 로드되면 이미지를 숨기고 비디오가 표시됩니다 -->
                <video id="shorts_main_video" src="<?php echo DOMAIN . $post_data['video']; ?>" controls
                    class="shorts_main_video width_box height_box" style="object-fit: cover; display:none; position: absolute; top: 0; left: 0;"></video>
            ...
            <div class="flex_ft font_px_015 shorts_content_1 padding_px-t_010">
                <div class="shorts_title_1 flv6 padding_px-u_010"><?php echo $post_data['title']; ?></div>
                <div class="shorts_contents_1"><?php echo nl2br($post_data['content']); ?></div>
            </div>

            <script nonce="<?php echo NONCE; ?>">
                // 비디오 및 이미지 요소
                var videoElement = document.getElementById('shorts_main_video');
                var imageElement = document.getElementById('shorts_main_img');
          ...
          FROM egb_like_log 
          WHERE like_target_table = 'egb_board_shorts'
          ...
          FROM egb_scrap_log 
          WHERE scrap_target_table = 'egb_board_shorts'
...
//댓글 출력
//프로팅 출력
auto_comment('egb_comment_shorts', $uniq_id, $board_user_uniq_id); 
auto_floating('egb_board_shorts', $uniq_id, 'shorts');

 //조회수 1증가
 $query = "UPDATE egb_board_shorts SET board_view = board_view + 1 WHERE uniq_id = :uniq_id";
