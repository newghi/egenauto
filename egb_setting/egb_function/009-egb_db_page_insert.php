...
function egb_db_page_insert($title, $name, $path, $type, $theme = 'eungabi') {
	try {
		// 중복 체크
		$check_query = "SELECT uniq_id FROM egb_page WHERE page_name = :page_name AND deleted_at IS NULL LIMIT 1";
		...
		$query = "INSERT INTO egb_page (
			...
			page_title, page_name, page_path, page_type, page_theme,
