...
//_setting 제거한 문자열 
$page_dir = str_replace('_setting', '', basename(dirname(dirname(dirname(__FILE__)))));
$page_dir_setting = basename(dirname(dirname(dirname(__FILE__))));

// 파일 경로 설정
$path_1 = ROOT . DS . 'egb_admin' . DS . 'admin_main_contents' . DS . $page_dir_setting . DS . 'menu_add' . DS . 'add_' . $page_name . '.php';
$path_2 = ROOT . DS . 'egb_admin' . DS . 'admin_main_contents' . DS . $page_dir_setting . DS . 'menu_edit' . DS . 'edit_' . $page_name . '.php';

$add_button = file_exists($path_1) ? '' : 'display_off';

// GET 파라미터 처리
$perPage = isset($_GET['per_page']) && ($_GET['per_page'] === '전체' || intval($_GET['per_page']) > 0) 
    ? $_GET['per_page'] 
    : 10;
...
// 총 레코드 수 가져오기 (검색 포함)
$totalQuery = "SELECT COUNT(*) as total FROM elephant_main_banner WHERE deleted_at IS NULL";
$params = [];

if (!empty($searchKeyword)) {
    $totalQuery .= " AND banner_title COLLATE utf8mb4_general_ci LIKE :search";
    $params[':search'] = '%' . $searchKeyword . '%';
}

$totalBinding = binding_sql(1, $totalQuery, $params);
$totalResult = egb_sql($totalBinding);
$totalRecords = isset($totalResult[0]['total']) ? $totalResult[0]['total'] : 0;
...
// 데이터 가져오기
$query = "SELECT * FROM elephant_main_banner WHERE deleted_at IS NULL";
if (!empty($searchKeyword)) {
            ...
            <div class="flex_fl">
                <div class="flex_fl_yc">
                    <div class="width_px_038 height_px_032 padding_px-x_009 padding_px-y_006 border_px-a_001 margin_px-r_005 pointer refresh_icon"
						...
						egbT('modal_<?php echo $page_dir; ?>_contents', 'id=modal_<?php echo $page_dir; ?>_contents_<?php echo $page_name; ?>&path=/egb/003-contents_box/modal_<?php echo $page_dir; ?>_contents&page_menu_path_name=<?php echo $page_dir; ?>&menu=<?php echo $page_name; ?>', true);">
...
document.getElementById('<?php echo $page_name; ?>_search_input_button').addEventListener('click', function () {
    ...
    const url = `id=modal_<?php echo $page_dir; ?>_contents_<?php echo $page_name; ?>&path=/egb/003-contents_box/modal_<?php echo $page_dir; ?>_contents&page_menu_path_name=<?php echo $page_dir; ?>&menu=<?php echo $page_name; ?>&page=1&per_page=${currentPerPage}&order=${currentOrder}&search_input=${encodeURIComponent(searchInput)}`;
...
(() => {
    ...
    if (perPageSelect) {
        ...
        perPageSelect.addEventListener('change', function () {
            ...
            const url = `id=modal_<?php echo $page_dir; ?>_contents_<?php echo $page_name; ?>&path=/egb/003-contents_box/modal_<?php echo $page_dir; ?>_contents&page_menu_path_name=<?php echo $page_dir; ?>&menu=<?php echo $page_name; ?>&page=${page}&per_page=${selectedValue}&order=${currentOrder}&search_input=${encodeURIComponent(searchValue)}`;
            egbInitializeMenu('<?php echo $page_dir; ?>');
            egbT('modal_<?php echo $page_dir; ?>_contents', url, true);
        });
    }
    ...
    if (orderSelect) {
        orderSelect.addEventListener('change', function () {
            ...
            const url = `id=modal_<?php echo $page_dir; ?>_contents_<?php echo $page_name; ?>&path=/egb/003-contents_box/modal_<?php echo $page_dir; ?>_contents&page_menu_path_name=<?php echo $page_dir; ?>&menu=<?php echo $page_name; ?>&page=${currentPage}&per_page=${perPageValue}&order=${selectedValue}&search_input=${encodeURIComponent(searchValue)}`;
            egbInitializeMenu('<?php echo $page_dir; ?>');
            egbT('modal_<?php echo $page_dir; ?>_contents', url, true);
        });
    }
            ...
            <div class="flex_fl_yc change_button_filter_off">
                ...
                <div class="padding_px-y_005 padding_px-x_020 border_px-a_001 border_bre-a_005 margin_px-x_010 pointer <?php echo $page_name; ?>_check_add <?php echo $add_button; ?>"
                    ...
                    egbT('add_<?php echo $page_name; ?>_contents_box', 'id=<?php echo $page_name; ?>_add&path=/egb/004-contents/<?php echo $page_dir_setting; ?>/side_menu_add/add_<?php echo $page_name; ?>&page_menu_path_name=<?php echo $page_dir; ?>&menu=<?php echo $page_name; ?>', true);">추가</div>
            </div>
        ...
        <div class="position4 z-index_2 max_width_9999" data-top="0%" data-left="0%">
            <div class="flex_fl width_box">
                ...
                <div class="flex_xc_yc width_box padding_px-y_010 border_px-y_002 border_px-x_001"
                    data-bg-color="#e5e5e5" data-bd-a-color="#d9d9d9">배너 제목</div>
            </div>
        </div>
        <?php
        // 결과 확인 및 반복 출력
                ...
                <label class="<?php echo $page_name; ?>_item" for="<?php echo $page_name; ?>_check_<?php echo $class_index; ?>"
                    ...
                    <div class="flex_fl width_box <?php echo $page_name; ?>_class_hover">
                        ...
                        <div class="flex_xc_yc width_box padding_px-y_010 border_px-u_002 border_px-x_001"
                            data-bg-color="#f9f9f9" data-bd-a-color="#d9d9d9"><?php echo $row['banner_title']; ?></div>
                    </div>
                </label>
                <?php
                $class_index++;
			...
			if (targetElement && targetElement.classList.contains('<?php echo $page_name; ?>_item')) {
				...
				if (checkbox) {
					...
					if (checkbox.checked) {
						...
						egbT('edit_<?php echo $page_name; ?>_contents_box', 'id=' + uniqId + '&path=/egb/004-contents/<?php echo $page_dir_setting; ?>/side_menu_edit/edit_<?php echo $page_name; ?>&uniq_id=' + uniqId + '&page_menu_path_name=<?php echo $page_dir ?>&menu=<?php echo $page_name; ?>', true);
						<?php endif; ?>
...
<div class="flex_xc_yc width_px_030 height_px_034 margin_px-x_002 border_px-a_001 pointer"
     ...
     <?php if ($page > 5): ?>
         egb:click="egbInitializeMenu('<?php echo $page_dir; ?>'); egbT('modal_<?php echo $page_dir; ?>_contents', 'id=modal_<?php echo $page_dir; ?>_contents_<?php echo $page_name; ?>&path=/egb/003-contents_box/modal_<?php echo $page_dir; ?>_contents&page_menu_path_name=<?php echo $page_dir; ?>&menu=<?php echo $page_name; ?>&page=<?php echo max($page - 5, 1); ?>&per_page=<?php echo $perPage; ?>&order=<?php echo $order; ?>', true);"
     <?php else: ?>
...
<div class="flex_xc_yc width_px_030 height_px_034 margin_px-x_002 border_px-a_001 pointer"
     data-bd-a-color="#d9d9d9" data-hover-bg-color="#dddddd"
     <?php if ($page > 1): ?>
         egb:click="egbInitializeMenu('<?php echo $page_dir; ?>'); egbT('modal_<?php echo $page_dir; ?>_contents', 'id=modal_<?php echo $page_dir; ?>_contents_<?php echo $page_name; ?>&path=/egb/003-contents_box/modal_<?php echo $page_dir; ?>_contents&page_menu_path_name=<?php echo $page_dir; ?>&menu=<?php echo $page_name; ?>&page=<?php echo $page - 1; ?>&per_page=<?php echo $perPage; ?>&order=<?php echo $order; ?>', true);"
     <?php else: ?>
         style="pointer-events: none; opacity: 0.5;"
     <?php endif; ?>>
    <div class="flex_xc_yc width_px_017 height_px_017 rotate_icon">
    ...
    for ($i = $startPage; $i <= $endPage; $i++): ?>
        <div class="flex_xc_yc width_px_030 height_px_034 margin_px-x_002 border_px-a_001 pointer <?php echo ($i == $page) ? 'pagination_choice_color' : ''; ?>"
             data-bd-a-color="#d9d9d9" data-hover-bg-color="#dddddd"
             egb:click="egbInitializeMenu('<?php echo $page_dir; ?>'); egbT('modal_<?php echo $page_dir; ?>_contents', 'id=modal_<?php echo $page_dir; ?>_contents_<?php echo $page_name; ?>&path=/egb/003-contents_box/modal_<?php echo $page_dir; ?>_contents&page_menu_path_name=<?php echo $page_dir; ?>&menu=<?php echo $page_name; ?>&page=<?php echo $i; ?>&per_page=<?php echo $perPage; ?>&order=<?php echo $order; ?>', true);">
            <?php echo $i; ?>
        </div>
    <?php endfor; ?>
<div class="flex_xc_yc width_px_030 height_px_034 margin_px-x_002 border_px-a_001 pointer"
...
<div class="flex_xc_yc width_px_030 height_px_034 margin_px-x_002 border_px-a_001 pointer"
     ...
     <?php if ($page <= $totalPages - 5): ?>
         egb:click="egbInitializeMenu('<?php echo $page_dir; ?>'); egbT('modal_<?php echo $page_dir; ?>_contents', 'id=modal_<?php echo $page_dir; ?>_contents_<?php echo $page_name; ?>&path=/egb/003-contents_box/modal_<?php echo $page_dir; ?>_contents&page_menu_path_name=<?php echo $page_dir; ?>&menu=<?php echo $page_name; ?>&page=<?php echo min($page + 5, $totalPages); ?>&per_page=<?php echo $perPage; ?>&order=<?php echo $order; ?>', true);"
     <?php else: ?>
         style="pointer-events: none; opacity: 0.5;"
     <?php endif; ?>>
    <div class="flex_xc_yc width_px_017 height_px_017">
