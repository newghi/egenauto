...
<div class="height_box overflow_y_auto scrollbar">
    <form id="egb_event_add_input" action="<?php echo DOMAIN . '/?post=egb_event_add_input'; ?>" method="post" enctype="multipart/form-data"
        ...
        <div class="padding_px-t_010 padding_px-x_010 padding_px-u_050">
            ...
            <div class="flex_ft padding_px-u_008">
                ...
                <select class="menu_input_style" name="event_type" id="event_type" required>
                    <?php
                    // 첫번째 카테고리의 이벤트 유형 옵션 조회
                    $first_category = !empty($tree) ? $tree[0]['label'] : '';
                    switch($first_category) {
                        case '회원정보':
                            echo '<option value="회원정보수정">회원정보수정</option>';
                            echo '<option value="평생회원">평생회원</option>';
                            break;
                        case '회원가입':
                            echo '<option value="쿠폰">쿠폰</option>';
                            echo '<option value="마일리지">마일리지</option>';
                            break;
                        case '구독료할인':
                            echo '<option value="연간구독">연간구독</option>';
                            echo '<option value="월간구독">월간구독</option>';
                            break;
                        default:
            ...
            <div class="flex_ft padding_px-u_008">
                ...
                <input class="menu_input_style" type="text" name="event_title" required>
            </div>
            
            <div class="flex_ft padding_px-u_008">
                <div class="padding_px-u_005">이벤트 설명</div>
                <textarea class="menu_input_style padding_px-x_010 padding_px-y_005" name="event_description"></textarea>
            </div>
            <div class="flex_ft padding_px-u_008">
                        ...
                        foreach ($community_grades as $grade) {
                            ...
                            <div class="flex_fl_yc padding_px-r_015 padding_px-u_005">
                                <input type="checkbox" name="target_grades[]" value="community_<?php echo $grade_id; ?>" 
                                       id="grade_<?php echo $grade_id; ?>" class="egb_checked cursor_pointer">
            ...
            <div class="flex_ft padding_px-u_008">
                ...
                <input class="menu_input_style" type="date" name="event_start_date" min="<?php echo $today; ?>" required>
            </div>
            
            <div class="flex_ft padding_px-u_008">
                <div class="padding_px-u_005">종료일</div>
                <input class="menu_input_style" type="date" name="event_end_date" min="<?php echo $today; ?>" required>
            </div>
        </div>
    </form>
</div>
